...
final class WOOF
{
    ...
    public function init()
    {
        ...
        if ($first_init != 1)
        {
            ...
            update_option('woof_show_sku_search', 1);
    ...
    public function woof_post_title_filter($where = '')
    {
        ...
        //if ($conditions)
        {
            if ($this->is_isset_in_request_data('woof_title'))
            {
                ...
                $search_by_title_behaviour = $this->settings['search_by_title_behaviour'];
                switch ($search_by_title_behaviour)
                {
                    case 'content':
                        $where .= "AND post_content LIKE '%{$woof_title}%'";
                        break;
                    case 'title_or_content':

                        $where .= "AND (post_title LIKE '%{$woof_title}%' OR post_content LIKE '%{$woof_title}%')";

                        break;
                    case 'title_and_content':
                        $where .= "AND (post_title LIKE '%{$woof_title}%' AND post_content LIKE '%{$woof_title}%')";
                        break;
                    default:
...
//***
        ...
        if (isset($this->settings['sku_submit_image']))
        {
            if (!empty($this->settings['sku_submit_image']))
            {
                ?>
                    .woof_sku_search_container .woof_sku_search_go{
                        background: url(<?php echo $this->settings['sku_submit_image'] ?>) !important;
                    }
                <?php
            }
        }
...
//***
        ...
        if (isset($this->settings['checked_color_img']))
        {
            if (!empty($this->settings['checked_color_img']))
            {
                ?>
                    .woof_color_term.checked{
                        background: url(<?php echo $this->settings['checked_color_img'] ?>) !important;
                    }
        ...
        $options = array
            ...
            array(
                ...
                'options' => array(
                    ...
                    2 => __('As drop-down', 'woocommerce-products-filter'),
                ),
                'desc_tip' => true
            ),
            array(
